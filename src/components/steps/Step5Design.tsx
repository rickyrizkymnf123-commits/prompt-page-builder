...
export function Step5Design({
  ...
  const updateTypo = (field: keyof DesignTypographyConfig, value: string) => {
    if (onChangeTypography) {
      onChangeTypography({ ...typography, [field]: value });
    }
  };

  return (
    <StepCard step={5} title="Visual, Warna Brand & Tipografi 🎨">
      ...
      <div className="space-y-2.5">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
          {colorSwatchList.map((swatch) => {
            const isSelected = warnaBrand === swatch.name || warnaBrandCustom === swatch.hex;
            return (
              <button
                key={swatch.name}
                type="button"
        ...
        <div className="flex items-center gap-3 p-2.5 rounded-xl bg-secondary/50 border border-border">
          <div className="flex items-center gap-2">
            <input
              type="color"
              value={warnaBrandCustom}
              onChange={(e) => handleSelectColor('Custom Color', e.target.value)}
              className="w-8 h-8 rounded-lg cursor-pointer border-0 bg-transparent p-0"
            />
            <span className="text-xs font-semibold text-foreground">Warna Kustom:</span>
          </div>
          <Input
            value={warnaBrandCustom}
            onChange={(e) => handleSelectColor('Custom Color', e.target.value)}
          ...
          <span className="text-[11px] text-muted-foreground hidden sm:inline">
            Pilih warna persis sesuai brand / logo produk Anda
          </span>
        </div>
      </div>
      ...
      <div className="space-y-2.5 pt-2">
        <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
          Tema Nuansa Halaman (Theme Style)
        </label>
      ...
      <div className="pt-2">
        <GroupedSelect
          label="Gaya Desain (Visual Archetype)"
          placeholder="Pilih gaya desain (Apple Style, Minimalist, Bento Grid)..."
          value={gayaDesain}
      ...
      <div className="space-y-3 pt-3 border-t border-border/60">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          {/* Button Size */}
          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase">Ukuran Tombol CTA</label>
            <select
              value={typography.buttonSize}
              onChange={(e) => updateTypo('buttonSize', e.target.value)}
              className="w-full h-9 rounded-xl bg-secondary border border-border px-2.5 text-xs font-medium text-foreground focus:outline-none focus:border-primary"
            >
              <option value="normal">Normal (Standar 44px)</option>
              <option value="large">Besar / Prominent (52px)</option>
              <option value="full">Full-Width Mobile (100% Layar)</option>
            </select>
          </div>

          {/* Entrance Animation */}
          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase">Animasi Masuk Section</label>
            <select
              value={typography.entranceAnimation}
